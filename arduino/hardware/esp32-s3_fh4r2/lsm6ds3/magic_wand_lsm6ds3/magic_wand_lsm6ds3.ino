/*
  magic_wand_lsm6ds3.ino: This program incorporates all accelerometer and
  gyroscope code of the Arduino Nano 33 BLE sense demo. Instead of the LSM9DS1
  the LSM6DS3 is used. All BLE and Tensorflow code is removed.
...
namespace {
  ...
  */
  ...
  int ReadAccelerometerAndGyroscope(int* new_samples) {
    // Keep track of whether we stored any new data
    *new_samples = 0;

    // Loop through new samples and add to buffer
    while (IMU.accelerationAvailable()) {
      const int gyroscope_index = (data_index % gyroscope_data_length);
      data_index += 3;
      float* current_gyroscope_data = &gyroscope_data[gyroscope_index];
    
      const int acceleration_index = (data_index % acceleration_data_length);

      float* current_acceleration_data = &acceleration_data[acceleration_index];
    
      // Read each sample, removing it from the device's FIFO buffer
      if (!IMU.readAccelerationAndGyroscope(
	...
	Serial.println("Failed to read acceleration and gyroscope data");
	return 0;
      }
      *new_samples += 1;
    }
  ...
  bool IsMoving(int samples_before) {
    ...
    const int start_index = ((data_index + 
    ...
    for (int i = 0; i < new_samples; ++i) {
      ...
      *done_just_triggered = ((old_state != eDone) && (*stroke_state == eDone));
      if (*done_just_triggered) {
        Serial.println("Done just triggered");
        Serial.print("Stroke length: ");
        Serial.println(stroke_length);
      ...
      if (*done_just_triggered) {
        ...
        for (i=0;i<10;i++) {
          Serial.print(gyroscope_data[i*3+start_index]);
          Serial.print(", ");
          Serial.print(gyroscope_data[i*3+1+start_index]);
          Serial.print(", ");
          Serial.println(gyroscope_data[i*3+2+start_index]);   
        }
      }
      ...
      if (*done_just_triggered) {
        ...
        for (i=0;i<10;i++) {
          Serial.print(orientation_data[i*3+start_index]);
          Serial.print(", ");
          Serial.print(orientation_data[i*3+1+start_index]);
          Serial.print(", ");
          Serial.println(orientation_data[i*3+2+start_index]);   
        }
      }
      ...
      if (*done_just_triggered) {
        Serial.print("current gravity x: ");
        Serial.print(current_gravity[0]);
        Serial.print(", y: ");
        Serial.print(current_gravity[1]);
        Serial.print(", z: ");
        Serial.println(current_gravity[2]);     
      ...
      if (*done_just_triggered) {
	      ...
	      if ((x_range < minimum_stroke_size) &&
	        ...
	        *stroke_transmit_length = 0;
	        stroke_length = 0;
          Serial.println("Stroke too small, canceled");          
  ...
  void printAccGyroData(float acc_x, float acc_y,float acc_z,
...
void setup() {
  ...
  Serial.println(" dps");
  // print register values related to the FIFO
...
void loop() {
  ...
  const bool data_available = IMU.accelerationAvailable();
  if (!data_available) {
    return;
  }
  int samples_read;
  ReadAccelerometerAndGyroscope(&samples_read);

  bool done_just_triggered = false;
  if (samples_read > 0) {
    EstimateGyroscopeDrift(current_gyroscope_drift);
    UpdateOrientation(samples_read, current_gravity, current_gyroscope_drift);
    UpdateStroke(samples_read, &done_just_triggered);
    EstimateGravityDirection(current_gravity);
    //UpdateVelocity(accelerometer_samples_read, current_gravity);
    // Serial.print("Stroke byte count: ");
