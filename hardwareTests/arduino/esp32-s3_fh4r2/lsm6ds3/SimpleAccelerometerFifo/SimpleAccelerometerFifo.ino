/*
  SimpleAccelerometerFifo.ino: Reads accelerometer data from the LSM6DS3 FIFO.
...
void setup() {
  ...
  while (!Serial);
  if (!IMU.begin()) {
    Serial.println("SimpleAccelerometerFifo.ino failed to initialize IMU!");

    while (1);
  }
  Serial.println("--------------------------------------------------------------------------");
  Serial.println("Verifying all IMU calls made by SimpleAcceremoterFifo.ino on the lsm6ds3");
  Serial.println("--------------------------------------------------------------------------"); 
