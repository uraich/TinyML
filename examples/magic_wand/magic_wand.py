...
class MagicWand(object):
    def __init__(self,debug=False):
        ...
        self.acceleration_data_length = const(600)
        self.acceleration_data = [(0.0, 0.0, 0.0)] * self.acceleration_data_length
        # The next free entry in the data array.
        self.acceleration_data_index = 0
        
        # A buffer holding the last 600 sets of 3-channel values from the gyroscope.
        self.gyroscope_data_length = const(600)
        self.gyroscope_data = [(0.0, 0.0, 0.0)] * self.gyroscope_data_length
        self.orientation_data = [[0.0, 0.0, 0.0]] * self.gyroscope_data_length
        # The next free entry in the data array.
        self.gyroscope_data_index = 0
    ...
    def setup(self):
        if self.debug:
            print("Setup started")
        self.IMU = LSM6DS3_IMU()
        self.acceleration_sample_rate = self.IMU.accelerationSampleRate()
        self.gyroscope_sample_rate = self.IMU.gyroscopeSampleRate()
        if self.debug:
            print("acceleration sample rate: {:4.1f}".format(self.acceleration_sample_rate))
            print("gyroscope sample rate: {:4.1f}".format(self.gyroscope_sample_rate))
        self.SetupIMU()
    ...
    def ReadAccelerometerAndGyroscope(self):
        ...
        for i in range(self.samples_read):
            self.gyroscope_data_index = self.gyroscope_data_index % self.gyroscope_data_length
            self.gyroscope_data[self.gyroscope_data_index] = gyro_samples[i]
            self.gyroscope_data_index += 1
            # print("index: ",self.gyroscope_data_index)
            self.acceleration_data_index = self.acceleration_data_index \
                                       % self.acceleration_data_length
            self.acceleration_data[self.acceleration_data_index] = acc_samples[i]
            self.current_acceleration_data = acc_samples[i]
            self.acceleration_data_index += 1
            
        if self.debug: 
            print("gyroscope data index: {:d}".format(self.gyroscope_data_index))
            print("New gyro values: x: {:4.2f}, y: {:4.2f}, z: {:4.2f}".format(
                self.gyroscope_data[self.gyroscope_data_index-1][0],
                self.gyroscope_data[self.gyroscope_data_index-1][1],
                self.gyroscope_data[self.gyroscope_data_index-1][2]))
        
            print("acceleration data index: {:d}".format(self.acceleration_data_index))       
            print("New acc values: x: {:4.2f}, y: {:4.2f}, z: {:4.2f}".format(
                self.acceleration_data[self.acceleration_data_index-1][0],
                self.acceleration_data[self.acceleration_data_index-1][1],
                self.acceleration_data[self.acceleration_data_index-1][2]))
        
    def VectorMagnitude(self,vec) :
        return math.sqrtf(vec[0] * vec[0] + vec[1] * vec[1] + vec[2] * vec[2])
    ...
    def EstimateGyroscopeDrift(self) :
        ...
        isMoving = VectorMagnitude(self.current_velocity) > 0.1
        if isMoving: 
            return
        ...
        start_index = self.gyroscope_data_index + \
            (self.gyroscope_data_length - samples_to_average) % self.gyroscope_data_length
        if self.debug:
        ...
        for i in range(samples_to_average):
            index = (start_index + i) % self.gyroscope_data_length
    ...
    def  UpdateOrientation(self,new_samples, gravity, drift) :
        if self.debug:
        ...
        start_index = (self.gyroscope_data_index + (self.gyroscope_data_length - new_samples)) % \
        ...
        recip_sample_rate = 1.0 / self.gyroscope_sample_rate
        ...
        previous_index = (index + self.gyroscope_data_length - 1) % self.gyroscope_data_length
    ...
    def IsMoving(samples_before):
        ...
        if self.gyroscope_data_index - samples_before < moving_sample_count:
            return False

        start_index = (gyroscope_data_index + \
                       (gyroscope_data_length - (moving_sample_count + samples_before))) % \
                       gyroscope_data_length

        total = 0.0
        for i in range(moving_sample_count):
            index = ((start_index + (i * 3)) % gyroscope_data_length)
            current_orientation = self.orientation_data[index]
            previous_index = (index + (gyroscope_data_length - 1)) % gyroscope_data_length
            previous_orientation = self.orientation_data[previous_index]
    ...
    def UpdateStroke(new_samples, done_just_triggered):
    ...
    def EstimateGravityDirection(self):
        if self.debug:
            print("Estimate Gravity Direction")
        samples_to_average = 100;
        if self.debug:
            print("acc index: {:d}".format(self.acceleration_data_index))
        if samples_to_average >= self.acceleration_data_index:
            samples_to_average = self.acceleration_data_index

        start_index = (self.acceleration_data_index + \
            (self.acceleration_data_length - samples_to_average)) % self.acceleration_data_length
        if self.debug:
        ...
        z_total = 0.0
        if self.debug:
            print("samples to avg: ",samples_to_average)
        for i  in range(samples_to_average):
            index = (start_index + i ) % self.acceleration_data_length
            if self.debug:
                print("index: {:d}".format(index))
                print(self.acceleration_data[index])
    ...
    def UpdateVelocity(self,new_samples, gravity):
        ...
        start_index = self.acceleration_data_index + \
            (self.acceleration_data_length - new_samples) % self.acceleration_data_length
    ...
    def loop(self):
        ...
        '''
            ...
            self.UpdateOrientation(self.samples_read,self.current_gravity,self.current_gyroscope_drift)
            # self.UpdateStroke(
            # accelerator updates
            self.current_gravity = self.EstimateGravityDirection()
        ...
        print("No of velocity values: {:d}".format(len(self.velocities)))
        for i in range(10):
            print(self.velocities[i])
        print("No of position values: {:d}".format(len(self.positions)))
        
        f = open("velocity.txt","w")
        for i in range(len(self.velocities)):
            f.write("{:8.4f}, {:8.4f}, {:8.4f}\n".format(self.velocities[i][0],
                                                         self.velocities[i][1],
                                                         self.velocities[i][2]))
        f.close()
        f = open("position.txt","w")
        for i in range(len(self.positions)):
            f.write("{:8.4f}, {:8.4f}, {:8.4f}\n".format(self.positions[i][0],
                                                         self.positions[i][1],
                                                         self.positions[i][2]))
        f.close()
        '''
        print("Data index: {:d}".format(self.acceleration_data_index))
        f = open("acceleration_data.txt","w")
        f.write("x, y, z\n")
        for i in range(self.acceleration_data_index):
            f.write("{:8.4f}, {:8.4f}, {:8.4f}\n".format(self.acceleration_data[i][0],
                                                       self.acceleration_data[i][1],
                                                       self.acceleration_data[i][2]))
        f.close()

        f = open("gyroscope_data.txt","w")
        f.write("x, y, z\n")
        for i in range(self.gyroscope_data_index):
            f.write("{:8.4f}, {:8.4f}, {:8.4f}\n".format(self.gyroscope_data[i][0],
                                                       self.gyroscope_data[i][1],
                                                       self.gyroscope_data[i][2]))
        f.close()
