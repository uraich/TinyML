...
def rasterizeStroke(stroke_points, x_range, y_range, width, height):
    ...
    for point_index in range(len(stroke_points) - 1):
        ...
        start_x_fp = norm_to_coord_fp(float(start_point["x"]), x_range_fp, half_width_fp)
        start_y_fp = norm_to_coord_fp(-float(start_point["y"]), y_range_fp, half_height_fp)
        end_x_fp = norm_to_coord_fp(float(end_point["x"]), x_range_fp, half_width_fp)
        end_y_fp = norm_to_coord_fp(-float(end_point["y"]), y_range_fp, half_height_fp)
...
for digit in range(10):
    ...
    rasterBuffer = rasterizeStroke(strokePoints, RANGE_X, RANGE_Y, RASTER_WIDTH,RASTER_HEIGHT)
    '''
    print("Colors:")
    for i in range(4):
        for j in range(3):
