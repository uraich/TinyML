# magic_wand_from_files.py: Reads the sample digit files and
# uses them to invoke the magic_wand model
...
# a byte array containing a 32x32 pixel image with 3 bytes per pixel
...
mnist_model_file = open ('models/magic_wand_model_quant.tflite', 'rb')

mnist_model = bytearray (31304)

mnist_model_file.readinto(mnist_model)
if len(mnist_model) != 31304:
...
for sample in range(10):
    sample_filename = "samples/{:d}.bin".format(sample)
    test_image_file = open (sample_filename, 'rb')
    
    test_image_file.readinto(test_image)
    print("Length of test image: {:d}, should be: {:d}".format(len(test_image),32*32*3))
